...
class ModifyProductTestCase(unittest.TestCase):
    """
        contains tests for THE ENDPOINT THAT MODIFIES A PRODUCT
    """
    def setUp(self):
        ...
        self.modify_product_details = {'name': 'Laser Jet New Model', 'unit_price': 500, 'quantity': 40}
    ...
    def test_that_an_admin_can_successfully_modify_a_product(self):
        ...
        data = json.loads(modify_product_response.data)
        self.assertEqual(modify_product_response.status_code, 200)
        self.assertEqual(data['product_id'], 1)
        self.assertEqual(data['name'], 'Laser Jet New Model')
    ...
    def test_that_a_sale_attendant_can_not_modify_a_product(self):
        ...
        data = json.loads(modify_product_response.data)
        self.assertEqual(modify_product_response.status_code, 403)
        self.assertEqual(
            data['error'],
